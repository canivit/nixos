...
in
{
  options.myModules.tmux = {
    ...
    searchPaths = lib.mkOption {
      ...
      default = [ ];
      description = "A list of paths to search for project directories";
    };
  };
  config = lib.mkIf cfg.enable (
    let
      combinedSearchPaths = lib.strings.concatStringsSep " " cfg.searchPaths;
      tmux-sessionizer = pkgs.writeScriptBin "tmux-sessionizer" (
        builtins.readFile (
          pkgs.substituteAll {
            src = ./tmux-sessionizer.sh;
            searchPaths = combinedSearchPaths;
            fd = "${pkgs.fd}/bin/fd";
